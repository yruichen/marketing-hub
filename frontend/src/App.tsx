...
import { CopyTab } from './features/generation/CopyTab';
// AI assistant mount — re-enabled after App.tsx slim-down.
// import {
//   AssistantBubble,
//   AssistantPanel,
//   PageContextTracker,
...
export default function App() {
  ...
  // Map an AssistantPanel `navigate(tab, ...)` event to the existing
  // routing + state pipeline. `tab` is the AppSection string the
  // Map AppSection to extra PageContext fields (so the assistant
  // knows whether the user is on a project detail view etc.).
  const _assistantContextByPath = useMemo<Record<string, unknown>>(
    () => ({
      '/': { tab: 'brainstorm' },

...
from harness.adapters.tools.context import ToolContext, ToolHandler, ToolNotAllowedError, ToolValidationError
...
class RegistryToolRuntime:
    ...
    async def execute(self, name: str, context: ToolContext, arguments: dict[str, Any]) -> dict[str, Any]:
        try:
            return await self._registry.get(name).invoke(context, arguments)
        finally:
...
def build_default_registry() -> ToolRegistry:
    ...
    from harness.adapters.tools.workspace.list_projects import build as build_list_projects
    from harness.adapters.tools.workspace.get_project import build as build_get_project
    from harness.adapters.tools.workspace.get_dashboard import build as build_get_dashboard
    from harness.adapters.tools.workspace.create_copy import build as build_create_copy
    from harness.adapters.tools.workspace.navigate import build as build_navigate

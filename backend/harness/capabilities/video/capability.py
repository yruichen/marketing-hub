...
from harness.capabilities._shared import _strip_json_fence, compact_text, platform_strategy
...
def build_video_generation_prompt(payload: dict[str, Any]) -> str:
    ...
    invariant = asset.user_prompt if asset else ''
    if not parts:
        production_brief = 'Create a neutral, executable commercial video brief.'
    else:
        production_brief = ' '.join(parts)
    if asset:
        return asset.render_user({'production_brief': production_brief})
...
def normalize_video_result(result: Any, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    video_topic = str(payload.get('video_topic') or result.get('video_topic') or '').strip()
    aspect_ratio = str(payload.get('aspect_ratio') or result.get('aspect_ratio') or '9:16').strip()
    video_url = extract_agnes_video_url(result) or str(result.get('video_url') or result.get('url') or '').strip()
    if not video_url:
        raise ValueError('The video provider completed without a playable video URL.')
    thumbnail_url = str(result.get('thumbnail_url') or result.get('poster_url') or '').strip()
